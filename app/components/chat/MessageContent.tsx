...
const PlotCarousel = dynamic(() => import('../charts/PlotCarousel'), { ssr: false });
...
function extractPlotlyCharts(content: string): { text: string; charts: any[] } {
  ...
  let cleanedContent = content.replace(codeBlockPattern, (match, lang, jsonStr) => {
    try {
      ...
      if (parsed.type === 'plotly_batch' && parsed.plots && Array.isArray(parsed.plots)) {
        charts.push({ type: 'batch', plots: parsed.plots });
        return '[PLOTLY_CHART]';
      }
      
      // check for single plots
      ...
      // bot a valid JSON or not a Plotly chart, keep original
    }
    return match;
  });
    ...
    <div className={currentProseClasses}>
      {parts.map((part, index) => (
        <div key={index}>
          ...
          {charts[index] && (
            charts[index].type === 'batch' ? (
              <PlotCarousel plots={charts[index].plots} />
            ) : (
              <PlotlyChart 
